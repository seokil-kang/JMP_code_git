...
H0=1e-1*eye(length(x0));          % initial guess for the inverse Hessian
...
if mcmc==1
    ...
    for i=2:M
        %if i==100*floor(.01*i);
        if mod(i,500) == 0
            acceptance_rate = round(mean(P(2:i,1)~=P(1:i-1,1))*1e4)/1e2;
            disp(['Now running the ',num2str(i),'th mcmc iteration (out of ',num2str(M),')' 'with acceptance rate = ',num2str(acceptance_rate),'%'])
        end
    ...
    fprintf('%.7f %.0f %.0f %.2f\n',fh,itct,retcodeh,r.mcmc.ACCrate*100)
end